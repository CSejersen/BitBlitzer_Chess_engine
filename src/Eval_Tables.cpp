...
void Eval_tables::print_table(int p){
  for(int i = 0; i < 64; i++){
    if(i % 8 == 0){
      std::cout << "\n";
    }
    std::cout << mg_table[p][i] << " ";
  }
}

int Eval_tables::get_mg_table(int p, int s){
  return mg_table[p][s];
}
int Eval_tables::get_eg_table(int p, int s){
  return eg_table[p][s];
}
int Eval_tables::get_gamephaseInc(int p){
  return gamephaseInc[p];
}
