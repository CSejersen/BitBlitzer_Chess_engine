        ...
        while (pieces) {
            ...
            switch (piece) {
                ...
                case nBlackKing:
                    ...
                    break;
                default:
                    throw std::invalid_argument("invalid piece in piece placement evaluation");
            }
        }
    }
...
int Search::search(int depth){
    ...
    for(int& move : moves){
        if(position->makeMove(move)){
            int max = -negamax(-INFINITY, INFINITY, depth - 1);
            if(max > bestScore){
                bestScore = max;
                bestMove = move;
            }
...
int Search::quiesce(int alpha, int beta) {
    ...
    // generate all starting moves
    moveGen->generateCaptures(captures, position->getWhiteToMove(), position->getCastlingRighs(),position->getEnPassantSquare());
...
int Search::negamax(int alpha, int beta, int depth ) {
    ...
    std::vector<int> moves;
    bool inCheck = position->getWhiteToMove() ? position->whiteInCheck : position->blackInCheck;
    moveGen->generateMoves(moves, position->getWhiteToMove(), position->getCastlingRighs(),
...
void Search::negamaxTest(int depth) {
    ...
    for (int &move: moves) {
        if (position->makeMove(move)) {
            int eval = -negamax(-INFINITY, INFINITY, depth - 1);
            std::cout << BitBoard::indexToCoordinate(getMoveStart(move)) << "-" << BitBoard::indexToCoordinate(getMoveTarget(move)) << " " << eval << std::endl;
            position->undoMove();
        }
    }
}
